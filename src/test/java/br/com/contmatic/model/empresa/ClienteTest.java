...
public class ClienteTest {
    ...
    public void deve_retornar_falso_na_comparacao_do_get_com_endereco_diferente_do_enviado_no_set() {
        Set<Endereco> endereco = new HashSet<>();
        endereco.add(Fixture.from(Endereco.class).gimme("valid"));
        assertThat(cliente.getEnderecos(), not(endereco));
    }

    /**
     * Deve retornar verdadeiro na comparacao do get com email igual do enviado no set.
     */
    @Test
    public void deve_retornar_verdadeiro_na_comparacao_do_get_com_email_igual_do_enviado_no_set() {
        assertThat(cliente.getEmail(), either(containsString("exemplo@teste.com")).or(containsString("testando@test.com")));
    }

    /**
     * Deve retornar falso na comparacao do get com email diferente do enviado no set.
     */
    @Test
    ...
    /**
     * Deve retornar falso quando comparado cliente a um objeto nulo atraves do equals.
     */
    @Test
    ...
    /**
     * Deve retornar falso quando comparado cliente a nulo atraves do equals.
     */
    @Test(timeout = 100)
    public void deve_retornar_falso_quando_comparado_cliente_a_nulo_atraves_do_equals() {
        assertFalse(cliente.equals(null));
    }

    /**
     * Deve retornar uma exception para cpf nulo.
     */
    @Test(expected = NullPointerException.class)
    public void deve_retornar_uma_exception_para_cpf_nulo() {
        cliente.setCpf(null);
    }

    /**
     * Deve retornar uma exception quando cpf e vazio.
     */
    @Test(expected = IllegalArgumentException.class)
    public void deve_retornar_uma_exception_quando_cpf_e_vazio() {
        cliente.setCpf("   ");
    }
    ...
    /**
     * Deve retornar uma exception quando inserido mais de um endereco.
     */
    @Test(expected = IllegalArgumentException.class)
    public void deve_retornar_uma_exception_quando_inserido_mais_de_um_endereco() {
        Set<Endereco> endereco = new HashSet<>();
        endereco.add(Fixture.from(Endereco.class).gimme("valid"));
        endereco.add(Fixture.from(Endereco.class).gimme("valid"));
        endereco.add(Fixture.from(Endereco.class).gimme("valid"));
    ...
    /**
     * Dece retornar uma exception quando inserido mais de um telefone.
     */
    @Test(expected = IllegalArgumentException.class)
    public void dece_retornar_uma_exception_quando_inserido_mais_de_um_telefone() {
        Set<Telefone> telefone = new HashSet<>();
        telefone.add(Fixture.from(Telefone.class).gimme("valid"));
        telefone.add(Fixture.from(Telefone.class).gimme("valid"));
        telefone.add(Fixture.from(Telefone.class).gimme("valid"));
    ...
    /**
     * Deve retornar verdadeiro se no to string contem o nome do cliente.
     */
    @Test
    public void deve_retornar_verdadeiro_se_no_toString_contem_o_nome_do_cliente() {
        assertThat(cliente.toString(), containsString("nome"));
    }

    /**
     * Deve retornar verdadeiro se no to string contem cpf.
     */
    @Test
    public void deve_retornar_verdadeiro_se_no_toString_contem_cpf() {
    ...
    public void deve_retornar_verdadeiro_na_captura_de_erro_quando_cpf_for_maior_que_o_permitido() {
        cliente.setCpf("123456789123456789123");
        Set<ConstraintViolation<Cliente>> constraintViolations = validator.validate(cliente);
        assertEquals(1, constraintViolations.size());
        assertEquals("CPF inválido", constraintViolations.iterator().next().getMessage());
    }
